...
def segLabelsforAxon(seg, gT, axonID):
    pred_ids, pred_counts = np.unique(seg[gT == axonID], return_counts=True)
    pred_counts = pred_counts[pred_ids > 0]  # don't take the 0, the bakckground
    pred_ids = pred_ids[pred_ids > 0]  # don't take the 0, the background
    return pred_ids


def segLabelsforAxonv2(output: np.ndarray, gt: np.ndarray) -> dict:
    """
    ...
    We create two  dictionnaries:
        ...
        Dict2: for each output ID we have: the nbr of such IDs N, z_max_ z_min, y_min, y_max,
            ...
            eg: Dict[3] = [128, 2, 4, 0,10, 5, 15, other_dict]. seg_ID 3 has 128 pixels, starting at z = 2 and finishing z = 4
                          start y: 4, finish y : 10
                          start x: 5, finish x: 15
                          new_dict[48] = 88 # 88 of the pixels of this seg_id correspond to axon 48 in GT
                          new_dict[52] = 5  # 5 of the pixels of this seg_id correspond to axon 52 in GT
    params:
    ...
    for index, gt_label in np.ndenumerate(gt): # enumerate all the pixels of gt, with the indexes for coords z,y,x
        output_label = output[index[0], index[1], index[2]] # corresponding pixel in the seg
        if output_label == 0:
        ...
        output_ids_stats[output_label][1] = max(index[0], output_ids_stats[output_label][1]) # z max of this ID
        output_ids_stats[output_label][2] = min(index[0], output_ids_stats[output_label][2]) # z min of this ID
        output_ids_stats[output_label][3] = max(index[1], output_ids_stats[output_label][3])  # y max of this ID
        output_ids_stats[output_label][4] = min(index[1], output_ids_stats[output_label][4])  # y min of this ID
        output_ids_stats[output_label][5] = max(index[2], output_ids_stats[output_label][5])  # x max of this ID
        output_ids_stats[output_label][6] = min(index[2], output_ids_stats[output_label][6])  # x min of this ID
        if gt_label == 0:
...
def smallSkelEndPoints(seg: np.ndarray, ID: int):
    """
    Compute the endpoints and the skeleton faster by looking only at the neighboorhood around ID
    :param seg:
    :param ID:
    :return: skeletons + list of endpoints [(z1, y1, x1)...]
    """
    coords = np.where(seg == ID)
    ...
    # We take a cube around the ID
    seg_bis = seg[z_min:z_max + 1, y_min:y_max + 1, x_min:x_max + 1]
    # We compute the skeleton of this ID
    skel = skeletonize(((seg_bis == ID) * 1))
    endPoints = findEndPointsBis(skel, z_min, y_min, x_min)
    return endPoints
def smallSkelEndPointsv2(seg: np.ndarray,z_min,y_min,x_min,ID: int):
    """
    Compute the endpoints and the skeleton faster by looking only at the neighboorhood around ID
    :param seg:
    :param ID:
    ...
    # We compute the skeleton of this ID
    #print('######################################################################')
    skel = skeletonize(((seg == ID) * 1))
    endPoints = findEndPointsBis(skel, z_min, y_min, x_min)

    #print('TIME 1: ' , t1)
    """
    def find_endpoints(matrix):
        skel = kimimaro.skeletonize(matrix)
        ...
        result_elements = unique_elements[np.where(counts == 1)]
        endPoints = []
        for endP in result_elements:
            endPoints.append((skel[1].vertices[endP].astype(np.int32)+[z_min, y_min, x_min]).tolist())
        return endPoints
        #endpoints = [i for i, degree in enumerate(skel_graph.degree()) if degree == 1]
        #return endpoints
    start = time.time()
    dipasse = find_endpoints((seg==ID)*1)
    t2 = time.time() - start
    print('TIME 2: ', t2)
    #print('#########')
    #print(endPoints)
...
def euclDist(p1, p2, treshold):
    ...
    """
    points = []
    # print(f"Nbr of points obj1: {len(p1)}")
    # print(f"Nbr of points obj2: {len(p2)}")
    for X1 in p1:
        for X2 in p2:
...
def findNeighbors(arr: np.ndarray, axon_ID: int) -> list:
    """
...
def check_neighboorhood(skel: np.ndarray, z: int, y: int, x: int) -> bool:
    ...
    if z < Z - 1 and y > 0 and x > 0 and skel[z + 1, y - 1, x - 1] == 1:
        ...
        if count >= 2:
            return False
    if z < Z - 1 and y > 0 and x < H - 1 and skel[z + 1, y - 1, x + 1] == 1:
        count += 1
        if count >= 2:
            return False
    return True